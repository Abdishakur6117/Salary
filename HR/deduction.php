...
<body class="hold-transition sidebar-mini layout-fixed">
            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Deduction List</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
  ...
  <!-- User Modal -->
            ...
            <div class="modal fade" id="deductionModal" tabindex="-1" role="dialog" aria-labelledby="userModalLabel" aria-hidden="true">
                    ...
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                          ...
                          <div class="modal-body">
                                        <form id="deductionForm" method="POST" action="" >
                                          <div class="row">
                                            <div class="col-md-6">
          ...
          <!-- insert/.modal -->
           <!--  update modal-->
            <div class="modal fade" id="edit_deductionModal" tabindex="-1" role="dialog" aria-labelledby="userModalLabel" aria-hidden="true">
              <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="userModalLabel">Update  Deduction</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
                          <div class="modal-body">
                                        <form id="edit_deductionForm" method="POST" action="" >
                                          <div class="row">
                                            <div class="col-md-6">
                                              <div class="form-group">
                                                <label for="employee">Employee Name </label>
...
<script src="../dist/js/pages/dashboard.js"></script>
  ...
  <script>
    ...
    function loadEmployee() {
        $.ajax({
            url: 'deductionOperation.php?action=get_employee',
            method: 'GET',
            dataType: 'json',
    ...
    $('#deductionForm').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: 'deductionOperation.php?action=create_deduction',
            data: $(this).serialize(),
            dataType: "json",
    ...
    $(document).on('click', '.editBtn', function() {
        ...
        $('#edit_id').val(allowanceData.id);
        $('#edit_employee_id').val(allowanceData.employee_id);
        $('#edit_amount').val(allowanceData.amount);
        $('#edit_type').val(allowanceData.type);
        $('#edit_description').val(allowanceData.description);
        $('#edit_date').val(allowanceData.date);
        
        $('#edit_deductionModal').modal('show');
    });
    // Update this part in your JavaScript
    $('#edit_deductionForm').submit(function(e) {
        e.preventDefault();
        ...
        $.ajax({
            url: 'deductionOperation.php?action=update_deduction',
            method: 'POST',
            data: formData,
            dataType: 'json',
            success: function(response) {
                if(response.status === 'success') {
                    showSuccess(response.message, function() {
                        $('#edit_deductionModal').modal('hide');
                        displayData();
                    });
                } else {
            ...
            complete: function() {
                submitBtn.prop('disabled', false).html('Update Deduction');
            }
        });
    });
    ...
    $(document).on('click', '.deleteBtn', function() {
        const deduction_id = $(this).data('id');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: 'deductionOperation.php?action=delete_deduction',
                    data: { id: deduction_id },
                    dataType: 'json',
                    success: function(res) {
                        if (res.status === 'success') {
                            showSuccess(res.message, function() {
    ...
    function displayData() {
        $.ajax({
            url: 'deductionOperation.php?action=display_deduction',
            dataType: 'json',
