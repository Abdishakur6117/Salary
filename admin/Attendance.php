...
<body class="hold-transition sidebar-mini layout-fixed">
  ...
  <!-- User Modal -->
            ...
            <div class="modal fade" id="attendanceModal" tabindex="-1" role="dialog" aria-labelledby="userModalLabel" aria-hidden="true">
          ...
          <!-- insert/.modal -->
           <!--  update modal-->
            <div class="modal fade" id="edit_attendanceModal" tabindex="-1" role="dialog" aria-labelledby="userModalLabel" aria-hidden="true">
              <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="userModalLabel">Update  Attendance</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
                          <div class="modal-body">
                                        <form id="edit_attendanceForm" method="POST" action="" >
                                          <div class="row">
                                            <div class="col-md-6">
                                              <div class="form-group">
                                                <label for="employee">Employee Name </label>
...
<script src="../dist/js/pages/dashboard.js"></script>
  ...
  <script>
    ...
    $('#attendanceForm').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: 'attendanceOperation.php?action=create_attendance',
            data: $(this).serialize(),
            dataType: "json",
    ...
    $(document).on('click', '.editBtn', function() {
        ...
        $('#edit_id').val(attendanceData.id);
        $('#edit_employee_id').val(attendanceData.employee_id);
        $('#edit_date').val(attendanceData.date);
        $('#edit_status').val(attendanceData.status);
        
        $('#edit_attendanceModal').modal('show');
    });
    // Update this part in your JavaScript
    $('#edit_attendanceForm').submit(function(e) {
        e.preventDefault();
        ...
        $.ajax({
            url: 'attendanceOperation.php?action=update_attendance',
            method: 'POST',
            data: formData,
            dataType: 'json',
            success: function(response) {
                if(response.status === 'success') {
                    showSuccess(response.message, function() {
                        $('#edit_attendanceModal').modal('hide');
                        displayData();
                    });
                } else {
    ...
    $(document).on('click', '.deleteBtn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: 'attendanceOperation.php?action=delete_attendance',
    ...
    function displayData() {
        $.ajax({
            url: 'attendanceOperation.php?action=display_attendance',
            dataType: 'json',
